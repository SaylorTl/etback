     ...
     <div id='box' class="menu-hide">
        <div class="worker station">
            ...
            <div v-show="search_more" class='condition-more clearfix box-width'>
                    <el-input v-model.trim="search.mobile" size="small" class="cell widthX150"  placeholder="电话"></el-input>
                ...
                <el-table v-loading="shade" element-loading-text="拼命加载中" :data="tableData" border fit style="width:100%">
                    ...
                    <el-table-column label="状态" width="80">
                        <template slot-scope="scope" >
                            <span :class="[{'green':scope.row.status==='use'}]">{{statusmap[scope.row.status]}}</span>
                        </template>
                    </el-table-column>
                   ...
                   <el-table-column label="操作" width="140">
                        <template slot-scope="scope">
                            <el-button v-if="scope.row.status==='use'" @click="endOrderClick(scope.row)" plain size="mini" ><i class="fa fa-pencil-square-o"></i>结束预约</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
...
export default {
    data:function(){
        return{
            statusmap:{"use":"使用中","cannel":"取消","end":"结束"},
            search:{station:'',dept:'',car_id:'',mobile:'',status:''},
            shade:false,
            search_more:false,
            tableData:[],
    ...
    methods: {
        endOrderClick(row){
            let vm = this;
            let url = '/lock/reserveUpdate';
            let info = {car_id:row.car,id:row.id,station_id:row.station}
        ...
        btnUndo:function(){
            this.search = {station:'',dept:'',car_id:'',mobile:'',status:''};
            this.pagination.page=1;
        ...
        getData:function(){
            var vm = this;
            vm.shade=true;
            var url = "/lock/reserveLists?page="+vm.pagination.page+"&pagesize="+vm.pagination.pagesize;
            let {station:station_id,mobile,dept,car_id,status} = vm.search;
            var searchQueryString={station_id,mobile,dept,car_id,status};
