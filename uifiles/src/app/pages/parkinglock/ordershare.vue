        ...
        <div class="worker station">
            ...
            <div class='table box-width'>
                <el-table v-loading="shade" element-loading-text="拼命加载中" :data="tableData" border fit style="width:100%">
                    ...
                    <el-table-column  label="大区/事业部" min-width="140">
                    ...
                    <el-table-column label="状态" width="50">
                        <template slot-scope="scope" >
                            <span :class="[{'green':scope.row.status==='issue'||scope.row.status==='use'},{'red':scope.row.status==='canceled'||scope.row.status==='end'}]">{{statusmap[scope.row.status]}}</span>

                        </template>
                    </el-table-column>
                   <el-table-column prop="time_begin" label="授权开始时间" min-width="100"></el-table-column>
                   <el-table-column prop="time_end" label="授权结束时间" min-width="100"></el-table-column>
                   <el-table-column prop="creationtime" label="创建时间" min-width="130"></el-table-column>
                </el-table>
            </div>
...
export default {
    data:function(){
        return{
            statusmap:{"issue":"发布中","use":"使用中","canceled":"已取消","end":"已结束"},
    ...
    methods: {
        ...
        getData:function(){
            var vm = this;
            vm.shade=true;
            var url = "/lock/shareLists?page="+vm.pagination.page+"&pagesize="+vm.pagination.pagesize;
