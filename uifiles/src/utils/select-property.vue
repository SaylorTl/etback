<template>
    <div ref="select_property" class="select-staion" :style="{ width: width }">
...
export default {
    ...
    methods: {
            ...
            if (this.value) {
                ...
                this.getStation(this.value, "id").then(function(data) {
                    if (typeof data != "undefined") {
                        ...
                        vm.$emit("select", { name: data.name, value: data.id });
                    }
                });
            } else {
                this.value2 = "";
            ...
            if (typeof vm.disabled == "undefined" || vm.disabled != true) {
                if (this.pophide) {
                    ...
                    this.$root.$el.addEventListener("click", function(e) {
                        if (!!vm.$refs.select_property) {
                            if (vm.pophide == false && vm.$refs.select_property.contains(e.target) == false) {
                                vm.pophide = true;
                            }
                        }
                    });
        ...
        inputChange: function() {
            ...
            if (newVal) {
                ...
                this.getStation(newVal, "keyword").then(function(json) {
                    vm.options =
                        json != "" && json.lists.length != 0
                            ? json.lists.map(function(k) {
        ...
        getStation: function(key, type) {
            var url = "";
            if (type == "id") {
                url = "/property/lists?property_id=" + key + "&page=1&pagesize=500";
            } else if (type == "keyword") {
                url = "/property/lists?name=" + key + "&page=1&pagesize=500";
            }
