    ...
    export default {
        ...
        created:function(){
            ...
            this.getDepartment().then(function(data){
                if(typeof(data) != 'undefined') vm.department = data;
            });
        },
        methods:{
            ...
            getDepartment:function(){
                var vm = this;
                var level = typeof(this.level) != 'undefined' ? this.level : 3;
                return utils.fetch("/department/tree?level="+level).then(function(json){
                    vm.loading = false;
                    if(typeof(json) != 'undefined' && json.code == 0){
                        return vm.resetStationId(json.content);
                    }
                });
            },
            resetStationId(arr){
                ...
                arr.forEach(item=>{
                    if(item.children){
                        vm.resetStationId(item.children);
                    }else{
                        if(item.level===3){
